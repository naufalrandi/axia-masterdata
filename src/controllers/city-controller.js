...
const getOne = async (req, res, next) => {
  try {
    const result = await cityService.getOne(req.params.slug);
    res.status(200).json({
      success: true,
      data: result,
    });
...
const update = async (req, res, next) => {
  try {
    const data = req.body;
    const result = await cityService.update(req.params.slug, data);
...
const destroy = async (req, res, next) => {
  try {
    const result = await cityService.destroy(req.params.slug);
    res.status(200).json({
      success: true,
      data: result,
    });
