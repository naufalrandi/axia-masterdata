...
const getOne = async (req, res, next) => {
  try {
    const result = await districtService.getOne(req.params.id);
    res.status(200).json({
      success: true,
      data: result,
    });
...
const update = async (req, res, next) => {
  try {
    const data = req.body;
    const result = await districtService.update(req.params.id, data);
...
const destroy = async (req, res, next) => {
  try {
    const result = await districtService.destroy(req.params.id);
    res.status(200).json({
      success: true,
      data: result,
    });
